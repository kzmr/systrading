...
3. **Executor Layer** (`app/Trading/Executor/`)
   - `OrderExecutor.php`: 注文実行とポジション管理
     ...
     - **決済逆指値注文**: トレーリングストップ/損切りを逆指値注文で管理（執行時はTaker）
...
GMOコインの手数料体系：
- **Taker手数料**: 0.05%（成行注文・逆指値注文の執行）
- **Maker手数料**: -0.01%（指値注文・リベート）

**重要: 現在の実装はエントリー・決済とも Taker です。**

決済は `stopSell` / `stopBuy`（逆指値注文）で管理しています。逆指値はトリガー価格
に達すると**成行で執行される**ため、Maker リベートは発生しません。

実測値（2026-08-11時点、BTC/JPY のクローズ済み175件）：

| | エントリー | 決済 | 往復 |
|---|---|---|---|
| BTC高値安値ブレイク戦略 | 0.0535% | 0.0529% | 約0.106% |
| BTC RSI逆張り戦略 | 0.047% | 0.047% | 約0.094% |

決済手数料の符号分布: マイナス（Makerリベート）**0件** / プラス（Taker）172件。

**動作仕様**:
- ポジション作成時に決済逆指値注文を自動発注
- トレーリングストップ更新時に逆指値価格も自動更新
- 緊急決済: 価格が逆指値から0.5%以上乖離した場合は成行で即決済
- フォールバック: 逆指値注文がない場合は成行注文で決済

**設計上の判断**: 決済を指値（LIMIT）にすれば Maker リベートを得られるが、
価格が飛んだ際に約定せず損失が無限に拡大しうる。逆指値を使うのは安全上の
意図的な選択であり、コストはその対価と考える。

コスト削減の余地があるのは**エントリー側**（現在は成行）。指値エントリーに
すれば往復 0.106% → 0.043% になるが、約定しない機会損失とのトレードオフが
あり未検証。
...
5. **決済逆指値注文**
   - トレーリングストップと損切りのうち、より保護的な価格で逆指値注文を発注
   - 価格更新時は既存の逆指値をキャンセルして再発注
   - **執行時はTaker手数料**（逆指値はトリガー後に成行執行されるため）

   ```
   ロング保有中:
   → 決済価格 = max(trailing_stop_price, stop_loss_price)
   → この価格でSELL逆指値(stopSell)注文を発注

   ショート保有中:
   → 決済価格 = min(trailing_stop_price, stop_loss_price)
   → この価格でBUY逆指値(stopBuy)注文を発注
   ```
...
- **決済逆指値注文管理**: 決済注文を逆指値で発注（安全側の設計、執行時はTaker手数料）
