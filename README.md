...
なおシグナルの発生頻度は想定（月4〜5回）を下回り、**実際は月1.7回**です。米国株が大きく下げる日自体が少ないためで、20取引での評価に到達するまで約12ヶ月かかる見込みです。閾値を緩めれば頻度は上がりますが、評価が意味を失うため変更していません。
...
- **日本プレミアムは予測指標ではなく値動きの「結果」**だった。当日騰落率との相関 -0.594、過去との相関（-0.325）が未来との相関（-0.261）より強い
...
同じ指標に対し、**過去の値動きとの相関**と**未来の値動きとの相関**を両方測ります。過去との相関のほうが強ければ、それは「結果」であって予測指標ではありません。

日本プレミアムはこれで否定されました（過去 -0.325 vs 未来 -0.261）。相関があること（t=-2.83）と、予測に使えることは別です。

**8. 探索に使ったデータで検証しない**

予備調査で「最有望」とした候補が、独立データで検証したら符号が真逆だった事例があります。探索に使ったデータは独立性を失っています。
